...
#if defined(NDEBUG)
# error "Uzythcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0731534e259e9ff48bd79ee1b33f5a1e38ffd6789fef670f297ff49636fe4ca5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uzythcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Uzythcoin Signed Message:\n";
...
{
    // Uzythcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Uzythcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Uzythcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Uzythcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Uzythcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Uzythcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0731534e259e9ff48bd79ee1b33f5a1e38ffd6789fef670f297ff49636fe4ca5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is my Uzyth coin 09/April/2019 ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd1f0da0a726f51ff624fee83ad48d7e5b5b84d46885364b9c2fb9c85237e98f8"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Uzythcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UzythcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UzythcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UzythcoinMiner(CWallet *pwallet)
{
    printf("UzythcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uzythcoin-miner");
    ...
    try { loop {
        ...
        printf("Running UzythcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UzythcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UzythcoinMiner, pwallet));
}
